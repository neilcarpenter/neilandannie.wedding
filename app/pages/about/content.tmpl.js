export default `
  <h1>So you guys are getting married?!</h1>
  <p>Yep! After 10 amazing years together, we’ve decided we’d quite like to stay this way.</p>
  <p>We’ll be doing the legal bit a few days before (with immediate family only) so we can have a really relaxed and personal ceremony with all the people we love at the Great Barn.</p>
  <p>We can't wait to celebrate with you all!</p>

  <h4>What’s happening on the day?</h4>
  <p>The ceremony starts at 1.30pm, and will either be outside in the orchard or in the barn (fingers crossed for sunshine!). </p>
  <p>After the ceremony we’ll have canapes and drinks in the orchard, then we'll sit down for dinner at 4pm.</p>
  <p>Following the speeches, the party starts around 6.30pm and ends at 11.30pm. Everyone has to leave the venue by midnight!</p>

  <h4>Can I bring my kids?</h4>
  <p>As much as we love your kiddies, unfortunately we are unable to invite them to our wedding. The only children at our day will be our gorgeous nieces and nephews.</p>

  <h4>Can I park my car at The Great Barn?</h4>
  <p>Yes, there is a free car park in the field next to the barn.</p>

  <h4>How do I find The Great Barn?</h4>
  <p>From the centre of Rolvenden take the A28 towards Tenterden. Just beyond the end of the 30mph limit turn Left into Halden Lane, the turning for which is adjacent to a row of houses. Drive for 1½ miles to the first farm on the Right, Halden Place (opposite Leyton House). Check the <a href="/when-where">When & Where page</a> for the full address.</p>

  <h4>Is there transport to and from the venue?</h4>
  <p>You’ll need to make your own way to and from the venue, but there are plenty of local taxi companies:</p>
  <ul>
    <li>007 Cars (Hawkhurst) - 01580 755775</li>
    <li>Adam’s Private Hire (Etchingham) - 07957 404201</li>
    <li>Central Cars (Staplehurst) - 01732 368988</li>
    <li>Elba Private Hire (Hawkhurst) - 07785 112021</li>
    <li>Hemsted Cars (Cranbrook) - 07707 242898</li>
    <li>Kent Cars Taxis (Goudhurst) - 01892 832285</li>
    <li>MTC Taxis (Staplehurst) - 01580 890003</li>
    <li>T&S Cars (Sandhurst) - 01580 851000</li>
    <li>Village Cabs (Wadhurst) - 01892 785711</li>
    <li>Zoos Private Hire (Hawkhurst) - 07542 660662</li>
  </ul>

  <h4>I need to book accommodation - can you recommend any hotels?</h4>
  <p>Check out the <a href="/places-to-stay">Places to stay</a> page for a list of local B&Bs and hotels.</p>

  <h4>What should I wear?</h4>
  <p>We aren't fussy - the usual wedding garb or whatever makes you feel comfortable! </p>
  <p>We hope to make full use of the lovely orchard for the ceremony and drinks, so choose footwear you can walk in on the grass!</p>

  <h4>Shall I bring a present?</h4>
  <p>We don’t expect gifts, but we know that some guests will want to get us something, for which we are really grateful.</p>
  <p>We’ve set up a gift list with xxxx where you can contribute to our honeymoon or buy us a present if you’d prefer.</p>
`
